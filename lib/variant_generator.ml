...
let remove_suffix s = List.hd (String.split_on_char '+' s)


let opam ~sandbox ~base prn ppf = Format.fprintf ppf
{| opam-version: "2.0"
...
|}
  prn (remove_suffix base) sandbox prn
...
let gen ~sandbox ~backport ~base pr =
 ...
 let ocaml = "https://github.com/ocaml/ocaml.git" in
  ...
  begin match backport with
   | None -> cmd "git checkout pr%d" pr
   | Some _ ->
     cmd "git fetch --tags";
...
let install_opam_file base sandbox pr =
  let cwd = Sys.getcwd () in
  let () =
    Sys.chdir
    @@ String.concat Filename.dir_sep
      [sandbox; "opam-repository"; "packages"; "ocaml-variants" ] in
  let loc = "ocaml-variants." ^ pr_variant_name base pr in
